...
class TutorialPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (gameType) {
      ...
      case 'chinese_checkers':    title = '中国跳棋 · 从零开始'; break;
  ...
  Widget _buildContent(bool night, ColorScheme cs) {
    ...
    switch (gameType) {
      ...
      case 'chinese_checkers': return _chineseCheckers(night, cs);
  ...
  //  中国跳棋
  // ═══════════════════════════════════════════════════════════════
  Widget _chineseCheckers(bool night, ColorScheme cs) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _section('欢迎来学中国跳棋！', [
        '中国跳棋（Chinese Checkers）是一种老少皆宜的经典棋类游戏。',
        '虽然叫"中国"跳棋，它其实起源于德国，原名 Stern-Halma。',
        '规则非常简单——把你的 10 个棋子从你的角跳到对面的角，先到者胜！',
      ], night, cs),

      _section('第一步：认识棋盘', [
        '中国跳棋的棋盘是一个六角星形，一共有 121 个孔位。',
        '棋盘有 6 个角，每个角是一个等边三角形营地，每个营地有 10 个孔位。',
        '你的 10 个棋子一开始就放在一个角里。',
        '游戏人数不同，开局使用的营地也不同：',
        '- 2 人对战：使用正对面的两个角',
        '- 3 人对战：每隔一个角使用一个（相隔 120°）',
        '- 4 人对战：使用两对正对面的角',
        '- 6 人对战：所有 6 个角全用上',
      ], night, cs),

      _section('第二步：游戏目标', [
        '🎯 把你的 10 颗棋子全部移到正对面的营地。',
        '谁先全部到位，谁就获胜！',
        '游戏没有"吃子"——棋子只会移动，永远不会被吃掉或移出棋盘。',
      ], night, cs),

      _section('第三步：怎么走棋？（两种走法）', [
        '',
        '走法一：相邻步（最简单）',
        '把你的棋子向 6 个可能的方向之一，移动到紧挨着的空位上。',
        '每次只能走一格。',
        '',
        '走法二：跳跃（核心玩法！）',
        '如果一个棋子的旁边有另一个棋子（不管是你的还是对手的），',
        '并且那个棋子另一侧的空位是空的，你就可以"跳"过去。',
        '就像跳棋，从一颗棋子上"翻"过去。',
        '',
        '⭐ 连跳：',
        '跳过去之后，如果新的位置旁边又有棋子可以跳，你可以继续跳！',
        '一次走棋中可以连续跳多次，只要条件满足。',
        '',
        '⚠️ 禁止的走法：',
        '- 不能跳过两个紧挨着的棋子',
      ...
      _section('第四步：多人模式说明', [
        '',
        '👥 2 人模式：你和 AI 各占一个正对面的角，最简单直接。',
        '',
        '👥👥 3 人模式：每隔一个角分配一个玩家。三人各占一角，互为对手。',
        '  中间区域比较开阔，策略上更注重长远规划。',
        '',
        '👥👥👥 4 人模式：使用两对对面角。局面更复杂，需要同时关注多个对手。',
        '',
        '👥👥👥👥👥👥 6 人模式：六角全满，最热闹的模式。',
        '  虽然是你一个人对 5 个 AI，但 AI 之间的相互牵制会给你创造机会。',
      ], night, cs),

      _section('第五步：新手常见误区', [
        '',
        '❌ 只知道走相邻步，不敢跳 —— 跳跃是跳棋的灵魂。',
        '  一旦你习惯用跳，会发现游戏速度快得多。',
        '',
        '❌ 只移动前面棋子，不管后面的 —— 后面的也需要往前赶。',
        '  尽量让所有棋子一起往前推进。',
        '',
        '❌ 跳得太远忘了留"桥" —— 你的棋子也是对手的"桥"。',
        '  注意不要把自己的棋子排成一条直路帮对手跳跃。',
      ], night, cs),

      _section('第六步：实用小技巧', [
        '🎯 连跳是最高效的前进方式——走棋前先看看能不能跳。',
        '🎯 开局先把营门口的路让开，不要堵在自己家门口。',
        '🎯 利用对手的棋子当"桥"来跳。',
        '🎯 在中间区域保留棋子作为"中转站"，可以加快前进速度。',
        '🎯 快到达对面时，注意最后几颗棋子怎么"进门"。',
      ], night, cs),
    ]);
  }
