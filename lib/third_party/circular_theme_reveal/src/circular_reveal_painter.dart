...
class CircularRevealPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double currentRadius = isReverse
    ...
    double opacity = 1.0;
    if (isReverse) {
      final double fadeThreshold = maxRadius * 0.15;
    ...
    paintImage(
        canvas: canvas, rect: imageRect, image: image, fit: BoxFit.cover);
    ...
    if (isReverse && currentRadius < maxRadius * 0.10) {
      ...
      final Paint glowPaint = Paint()
        ..color = Colors.white.withValues(alpha: glowIntensity * 0.25)
